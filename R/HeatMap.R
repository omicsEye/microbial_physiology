...
all.heatmap <- function(data, sample_meta, feature_meta, percentile = 0.75, show = FALSE, omit_na = FALSE, file_location ='') {
  
  foreach(i = 1:length(feature_meta)) %do% {
    ggsave(
      filename = paste0(file_location, names(feature_meta)[i], '.png'),
      create.heatmap(data = data, 
