...
create.heatmap <-
  function(data, sample_meta, feature_meta, omit_na = TRUE) {
...
one.v.all <-
  function(data, sample_meta, feature_meta, which = 2, column, trait) {
    meta <- ""
    ...
    return(create.heatmap(data, sample_meta, feature_meta, omit_na = FALSE))
  }

# create all possible heatmaps for a certain feature
all.one.v.all <- 
  function(data, sample_meta, feature_meta, which = 2, column) {
