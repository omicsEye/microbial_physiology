...
GetTrait <- function(col_names, current_column) {
  ...
  trait <- gsub('[[:punct:]]', ' ', substring(trait, regexpr("\\.",trait)[[1]][1] + 1))
  return(trait)
}

# Organize ProTrait table into clean format
CleanProTrait <- function(save_file=TRUE) {
  column_names <- c("specie", "metabolite usage", "enzymes", "ecosystem", "pH", 
  ...
  if(!file.exists('ProTraits_binaryIntegratedPr0.90.txt')) {
    download.file('http://protraits.irb.hr/data/ProTraits_binaryIntegratedPr0.90.txt', 'ProTraits_binaryIntegratedPr0.90.txt')
  }
  raw_table <- read.delim('ProTraits_binaryIntegratedPr0.90.txt', fill=NA, stringsAsFactors = FALSE)
  # only want first 313 columns
  raw_table <- raw_table[,1:313]
  
  num_rows <- nrow(raw_table)
  num_cols <- ncol(raw_table)
  
  refined_table <- data.frame(matrix(nrow = num_rows, ncol = length(column_names)))
    ...
    while(current_column <= num_cols) {
      ...
      if(!is.na(current_cell) & current_cell == 1) {
        ...
        if(current_column >=3 & current_column <= 111) {
          ...
          # get ecosystem
          if(grepl("ecosystem", current_name) | grepl("habitat", current_name)) {
          ...
          if(grepl("gram_stain", current_name)) {
            meta_cell <- refined_table[current_line, "gram stain"]
  ...
  if(save_file) {
    write.csv(refined_table, "CleanProTrait.csv", row.names = FALSE)
  }
